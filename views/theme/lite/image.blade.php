...
@section('content')
<article>
    <p><strong>{{ ucwords($keyword) }}</strong>. {{ $sentences->shuffle()->take(3)->implode(' ') }}</p>
    @php
    ...
    @foreach(collect($images)->shuffle()->take(1) as $image)
    <figure>
         <img alt="{{ $image['title'] }}" src="{{ $image['url'] }}" width="100%" onerror="this.onerror=null;this.src='{{$img_thumb}}';">
        <small>{{ $image['title'] }} | Source: {{ $image['domain'] }}</small>
        <p>{{ $sentences->shuffle()->take(5)->implode(' ') }}</p>
    </figure>
    @endforeach
